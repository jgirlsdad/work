...
get_ipython().system('pip install pyflowchart')
...
##  get cdos nonprofit charity definition file name to list
    ...
    for line in lines[1:]:
        ...
        title = a[1].strip()
        print(f"ds :{title}:")
        sourceFieldDicts[title]=dictn
    
    
    global dataSetsEtl,datasets,groupMenu,runEtls
    groups = []
...
##  Process the run_etl.json files
 ...
 #   for val in dataSets:
        if "title" in val:
          ...
          dataSetsEtl[title]["info"]["directory"] =bdir
          dataSetsEtl[title]["info"]["group"] = groupNew
          dataSetsEtl[title]["info"]["datasetCharId"] = ""
          if title in sourceFieldDicts:
                dataSetsEtl[title]["info"]["sourceFieldDict"]=sourceFieldDicts[title]
          else:
                dataSetsEtl[title]["info"]["sourceFieldDict"]= ""
     ...
     #     dataSetsEtl[title]["info"]["groups"] = info[title]["groups"]
          ...
          dataSets.append(val)
          groups.append(ll)
        dataSetsEtl[title]['extract'] = {}
        if 'extract' in val:
            
            dataSetsEtl[title]['extract']["localFiles"] = []
            
            if isinstance(val["extract"],dict):
                extract,datasetCharId,localF =  extractDecode(val["extract"])
                dataSetsEtl[title]['extract']["localFiles"].append(localF)
            elif isinstance(val["extract"],list):
                extract=""
                dataSetsEtl[title]["info"]["datasetCharId"] = "n/a"
                for e in val["extract"]:
                    extr,datasetCharId,localF = extractDecode(e) 
                    extract+=extr + ";"
                    dataSetsEtl[title]['extract']["localFiles"].append(localF)
                    
                    if len(datasetCharId) > 2:
                        dataSetsEtl[title]["info"]["datasetCharId"] = datasetCharId
        else:
            extract=""
       
        dataSetsEtl[title]['extract']['job'] = extract
        transform=""
        filetr = ""
        if 'transform' in val:
            if val["transform"]["language"] == "node":
                 ff=val["transform"]["file"]
                 transform=f"node {bicHome}{directory}/{ff}"
            elif val["transform"]["language"] == "py":
                 ff=val["transform"]["file"]
         ...
         #   filetr=ff    
            options=""
            if 'options' in val['transform']:
                for opts in val['transform']['options']:
                    options+= f" {opts}" 
            ...
            transform=""
        dataSetsEtl[title]["transform"] = transform
        dataSetsEtl[title]["info"]['filetr'] = filetr
...
def getETL(title):
    print(f"Getting info:{title}:")
    extract = dataSetsEtl[title]['extract']['job']
    transform = dataSetsEtl[title]['transform']
    directory = dataSetsEtl[title]['info']['directory']
    datasetCharId = dataSetsEtl[title]['info']['datasetCharId']
    filetr = dataSetsEtl[title]['info']['filetr']
...
def getXrefs():
    ...
    sheet = client.open('BIC Data Inventory and Metadata').worksheet('Maintenance_Framework')
    repo_sheet = client.open('BIC Data Inventory and Metadata').worksheet('MetadataRepository')
    fields_sheet = client.open('BIC Data Inventory and Metadata').worksheet('Field Descriptions')
...
def showRecentLogs(errorsByDate):
 ...
 #   print("XX XX XX",xx)
    ...
    layout = [
        ...
        [sg.Table(values=xx,headings=header,visible_column_map=[True,True,True,True,True,False,False], size=(120, 20),row_height=40,row_colors=colrw,vertical_scroll_only=False,max_col_width=60,enable_events=True, key='-DAILY-',col_widths=colWidths)],
...
def tkPopupWrite(w4x4,string,title,outfile,bgColor="grey",mapping={}):
    toWrite=False
    def close():
       popup.destroy()
       popup.quit()
    
    def toFile(outfile,mapping):
       now = datetime.today()
       print("Outfile in toFile: ",outfile)
       if (os.path.isfile(outfile)):
          string=(f"Source Field Defintion File Exists: {outfile}\n\n")
          of = outfile.split("/")
          off = f"/tmp/{of[-1]}-{now.year}-{now.month}-{now.day}-{now.hour}-{now.minute}-{now.second}"
          string+=(f"Copying Existing Source Field Definition TO: {off}")
          shutil.copyfile(outfile,off)
          sg.Popup(string,font=('Courier',20))
#       with open(f"{w4x4}_source_field_defintions.json","w") as jfile:
       with open(outfile,"w") as jfile:
            jfile.write(json.dumps(mapping))
       ti_c = os.path.getctime(outfile)
       ti_m = os.path.getmtime(outfile)
       ti_c = time.ctime(ti_c)
       ti_m = time.ctime(ti_m)
       sz = os.path.getsize(outfile)
       sg.Popup(f"Library written to {outfile}\nSize: {sz}\nCreation Time: {ti_c}\nModfile Time: {ti_m}\n",font='Courier 15')
       print(f"Library written to {outfile}\nSize: {sz}\nCreation Time: {ti_c}\nModfile Time: {ti_m}\n")
        

       popup.destroy()
       popup.quit()
    print("OUtfile in Fun ",outfile)
    popup = tk.Tk()
    popup.wm_title(title)
    label = Label(popup, text=string,font=('Courier',15),justify="left", relief="groove")
    label.configure(bg=bgColor)
    label.pack(side="top", fill="x", pady=10)
    my_button_w= Button(popup, text= "Write", font=('Courier',25),
    borderwidth=2, command=lambda: toFile(outfile,mapping))
    my_button_w.pack(pady=20)
    
    my_button= Button(popup, text= "Cancel", font=('Courier',15),
    borderwidth=2, command= close)
    my_button.pack(pady=20)
    popup.mainloop()
...
def showSrc2TransXrefs(file):
    ...
    [sg.Button("Write XREF File")],rows     
    ]
...
def analyzeFieldDefs(file):
    ...
    string+="Fields in DATA not found in LIBRARY\n"
    if len(notInLib) > 0:
    ...
    sg.popup(string,font="CENTAUR 15 bold")
    ...
    for k,v in values.items():
        ...
        if k[:4] == "DEFS" and k[6:10] == "CUST":
            ...
            if k[6:10] == "CUST":
                ...
                if v not in mapping:
                    mapping[v] = []
                mapping[v].append(dfs)
       
                
    print("DEFS",defs)
    print("FLDS",flds)
##  Add Non-Empty Custom Fields    
    for nn,val in defs.items():
        print(f"CVUSTOMS  :{nn}: :{val}:{flds[nn]}")
        if flds[nn] not in mapping:
            mapping[flds[nn]] = []
        mapping[flds[nn]].append(val)
  ...
  #         finalMap[v[0]] = col
    ...
    if len(stringBad) > 0:
        ...
        outFile = os.path.join(targDir,f"{w4x4}_src_fld_defs.json")
        print("OUTFILE ",outFile)
        tkPopupWrite(w4x4,stringGood,"Creating Source Definitions",outFile,"white",finalMap)
 #       tkPopScroll(stringGood,toFile,outFile,finalMap)
    return outFile

def sourceDefs2FieldsEdit(w4x4,title,library={}): 
    ...
    if len(library) > 0:
        row=[]
        row.append(sg.Text("FIELD",font='Courier 15',size=(30,1)))
        row.append(sg.Text("DEFINITION",font='Courier 15',size=(70,1)))
        rows.append([sg.Frame("",[row])])
        for fld,definition in library.items():
            row=[]
        #    row.append(sg.Input(default_text=fld,font='Courier 15',key=f"FLD:{fld}",size=(30,1)))
            row.append(sg.Input(default_text=fld,font='Courier 15',key=f"FLDS: {definition}",size=(30,1)))
            
            row.append(sg.Input(default_text=definition,font='Courier 15',key=f"DEFS: {fld}",size=(70,3)))
            rows.append([sg.Frame("",[row])])
    ...
    row.append(sg.Text("FIELD",font='Courier 15',size=(30,1)))
    row.append(sg.Text("DEFINITION",font='Courier 15',size=(70,1)))
    rowsC.append([sg.Frame("",[row])])
    if len(library) > 0:
        nrows =5
    else:
        nrows = 20
## Append Some Empty Rows for Adding elements
    for nn in range(nrows):
          row=[]
          row.append(sg.Input(default_text="",font='Courier 15',key=f"FLDS: CUST:{nn+1}",size=(30,1)))
          row.append(sg.Input(default_text="",font='Courier 15',key=f"DEFS: CUST:{nn+1}",size=(70,3)))
          rowsC.append([sg.Frame("",[row])])
        
    layout = [[sg.Text(title,font='Courier 15 bold')],
    [sg.Button("Quit")],
    ...
    [sg.Sizer(0,500),sg.Column(rows,scrollable=True,size=(1200,400))],
    [sg.Frame("Add New Source Field Definitions",[[sg.Column(rowsC,scrollable=True)]],font='Courier 15')]
    ]
  
    sg.theme("LightBrown3")     
    window2 = sg.Window("Source Fields and Definitions",layout,finalize=True,resizable=True)
    a = window2.CurrentLocation()
    screen_width, screen_height = window2.get_screen_dimensions()
    win_width, win_height = window2.size
    x, y = (screen_width - win_width)//2, (screen_height - win_height)//2
...
def sourceDefsANDFields(dfile,fields): 
    '''Reads the new tsv files that include the Field AND teh Defintion
    '''
    global mapping
    ...
    for nn,col in dfF.sort_values(by="Fields").iterrows():
        row=[]
        field = col['Field']
        definition = col['Definition']
        row.append(sg.Text(field,  
                            font='Courier 15 bold ',key=f"FLDS: {field}",
                            size=(30,3)))
        row.append(sg.Text(definition,font='Courier 15',key=f"DEFS: {definition}",size=(30,3)))
...
def sourceDefs2Fields(dfile,fields): 
    ...
    for nn,col in enumerate(defs):
        ...
        row.append(sg.Combo(fieldsTmp,default_value=tmpD2F[col],enable_events=True,  
                            font='Courier 15 bold ',key=f"FLDS: {col}",
                     #       select_mode="LISTBOX_SELECT_MODE_SINGLE",
                            size=(30,6)))
    #    row.append(sg.Input(default_text=fld,font='Courier 15',key=f"FLD:{fld}",size=(30,1)))
    #    row.append(sg.Input(default_text=definition,font='Courier 15',key=f"DEFS: {fld}",size=(70,3)))
        row.append(sg.Text(col,font='Courier 15',key=f"DEFS: {col}",size=(30,3)))
    ...
    layout2 = [    
                   ...
                   [sg.Button('Write Unique'),
                    sg.Table(values=valuesUNQ,
                       ...
                       auto_size_columns=False,enable_events=True,def_col_width=30,text_color="black",
...
def getDefinitionFile():
    '''  Prvides a file browser so user can select a source definition file, which just lists the definitions
    '''

    layout = [ [sg.Text("Choose the file that contains the Source Fields and their Definitions")],
               [sg.Button("Quit")],
               [sg.FilesBrowse(button_text="Local File",initial_folder="/home/joe/work/CDOS/nonprofit/defs",font="CENTAUR 15",file_types=[("TSV Files","*.tsv"),("CSV Files","*.csv"),("Excel Files","*.xlsx")],enable_events=True,key='-FILEDEF-')]
...
def toolTips():
    global createSourceFieldDefsTip,getSourceDefsTip
    
    createSourceFieldDefsTip = "Create a Source Fields Definition Dictionary.  This requires a Source Definition File AND that the Datasets Source Fields have been Extracted and Analyzed"
    getSourceDefsTip ="Read the Source Defintions file that was created manually"
...
def generateColor(flag,rmin=0,rmax=255,gmin=0,gmax=255,bmin=0,bmax=255):
    '''  If Flag == "G", generatea shade of Green ,fi "R", generate a shde of Red
     and if "B", generate a shade of Blue'''
...
def cronGUI():
    ...
    global dfExtract,dfTransform,directory,values,targDir
    global sourceDefFile,nfieldsadded,nfieldsAddedS
    global createSourceFieldDefsTip,getSourceDefsTip,dataSetsEtl
...
## Source Field Definition Buttons
    sourceFieldButtons = [[
     sg.Button("Get NEW Src Defs/Flds",font='Courier 15 bold ',visible=True,tooltip=getSourceDefsTip)    
     sg.Button("Get Source Defs",font='Courier 15 bold ',visible=True,tooltip=getSourceDefsTip),
     sg.Button("Show/Edit Source-Field Defs",font='Courier 15 bold '),
     sg.Button(f"Create Source-Field\nDefs from Scratch",key="Create Source Field Defs from Scratch",font='Courier 12 bold '),                      
     sg.Button("Create Source Field Defs",font='Courier 15 bold ',visible=False,tooltip=createSourceFieldDefsTip),
     sg.Button("Analyze Source Defs",font='Courier 15 bold')]]
    
    
    layout = [
              [sg.Text("BIC Cron DataSets")],
              [sg.Combo(datasets,enable_events=True,key="-DATASET-",font='Courier 15 bold ')],
              ...
              [sg.Button("Get Error Logs",font='Courier 15 bold ',),sg.Button('Show Recent Errors',font='Courier 15 bold ',visible=False),sg.Button("Update Logs",font='Courier 15 bold ',visible=False),sg.Button("View Log Summary",font='Courier 15 bold ')],
   ...
   #          [sg.Button("GO")],
              ...
              [sg.Frame("Source Field Definitions",sourceFieldButtons,font='Courier 15 bold ')],
              ...
              [sg.Multiline(default_text="Dataset Summary",enable_events=True,font='Courier 15 bold ',background_color="lightblue",key="-OUTPUT-",size=[70,10]),
               sg.Multiline(default_text="ETL Summary",key="-ETL-",size=[70,20],font='Courier 15 bold ')]             
            ]
#sourceDefsANDFields(dfile,fields)
    print("Step 1")   
    # Create the Window
...
### Show/Edit Source Field Definitions
...
### Create Source Field Defs from Scratch            
...
values = {'SOURCE:Entity Id': 'Entity Id', 'LISTBOX:Entity Id': 'entityId', 'DESC:Entity Id': 'Entity ID', 'NOTES:Entity Id': '', 'DATE:Entity Id': '2024-1-18', 'SOURCE:Document Id': 'Document Id', 'LISTBOX:Document Id': 'documentId', 'DESC:Document Id': 'Document Identifier Number', 'NOTES:Document Id': '', 'DATE:Document Id': '2024-1-18', 'SOURCE:Ce Fein': 'Ce Fein', 'LISTBOX:Ce Fein': 'fein', 'DESC:Ce Fein': 'Federal Employer Identification Number', 'NOTES:Ce Fein': '', 'DATE:Ce Fein': '2024-1-18', 'SOURCE:Org Name': 'Org Name', 'LISTBOX:Org Name': 'name', 'DESC:Org Name': 'Charitable Organization, Paid Solicitor or Professional Fundraising Consultant name', 'NOTES:Org Name': '', 'DATE:Org Name': '2024-1-18', 'SOURCE:Cs State': 'Cs State', 'LISTBOX:Cs State': 'stateAbbreviation', 'DESC:Cs State': 'State abbreviation', 'NOTES:Cs State': '', 'DATE:Cs State': '2024-1-18', 'SOURCE:Cs Auth Flg': 'Cs Auth Flg', 'LISTBOX:Cs Auth Flg': 'authorizedSoliciting', 'DESC:Cs Auth Flg': 'If soliciting authorized in state, then ‘Y’.  If soliciting suspended in state, then ‘N’', 'NOTES:Cs Auth Flg': '', 'DATE:Cs Auth Flg': '2024-1-18', 'SOURCE:Entity Type': 'Entity Type', 'LISTBOX:Entity Type': 'registrantTypeAbbr', 'DESC:Entity Type': 'Entity type', 'NOTES:Entity Type': '', 'DATE:Entity Type': '2024-1-18', 'SOURCEC:CUST:1': '', 'TRANSC:CUST:1': '', 'DESCC:CUST:1': '', 'NOTESC:CUST:1': '', 'DATEC:CUST:1': '2024-1-18', 'SOURCEC:CUST:2': '', 'TRANSC:CUST:2': '', 'DESCC:CUST:2': '', 'NOTESC:CUST:2': '', 'DATEC:CUST:2': '2024-1-18', 'SOURCEC:CUST:3': '', 'TRANSC:CUST:3': '', 'DESCC:CUST:3': '', 'NOTESC:CUST:3': '', 'DATEC:CUST:3': '2024-1-18', 'SOURCEC:CUST:4': '', 'TRANSC:CUST:4': '', 'DESCC:CUST:4': '', 'NOTESC:CUST:4': '', 'DATEC:CUST:4': '2024-1-18', 'SOURCEC:CUST:5': '', 'TRANSC:CUST:5': '', 'DESCC:CUST:5': '', 'NOTESC:CUST:5': '', 'DATEC:CUST:5': '2024-1-18'}


# In[ ]:


writeFieldFileN2(cim4x4,"Other State Solicitation of Charities’ Registrants in Colorado",values,targDir,"")
...
dataSetsEtl["Other State Solicitation of Charities’ Registrants in Colorado"]
#dataSetsEtl["Registration of Charities, Paid Solicitors, Professional Fundraising Consultants, and for-profit Public Benefit Corporations in Colorado"]
...
for title in dataSetsEtl.keys():
    if 'localFiles' in dataSetsEtl[title]['extract']:
        print(dataSetsEtl[title]['extract']['localFiles'])
...
# Which you can wrap in a list() constructor to materialize
 ...
 #   for val in dataSets:
        if "title" in val:
          ...
          dataSetsEtl[title]["info"] = {}
          dataSetsEtl[title]["info"]["directory"] =bdir
          dataSetsEtl[title]["info"]["group"] = group
          dataSetsEtl[title]["info"]["datasetCharId"] = ""
     #     dataSetsEtl[title]["info"]["groups"] = info[title]["groups"]
          nit=0
          ll = go_deeper(ll,title,nit)
        ...
        if 'extract' in val:
            if isinstance(val["extract"],dict):
                extract,datasetCharId =  extractDecode(val["extract"])

            elif isinstance(val["extract"],list):
                extract=""
                dataSetsEtl[title]["info"]["datasetCharId"] = "n/a"
                for e in val["extract"]:
                    extr,datasetCharId = extractDecode(e) 
                    extract+=extr + ";"
                    if len(datasetCharId) > 2:
                        dataSetsEtl[title]["info"]["datasetCharId"] = datasetCharId
        else:
            extract=""
        dataSetsEtl[title]["extract"] = extract
        transform=""
        filetr = ""
        if 'transform' in val:
            if val["transform"]["language"] == "node":
...
def getETL(title):
    print(f"Getting info:{title}:")
    extract = dataSetsEtl[title]['extract']
    transform = dataSetsEtl[title]['transform']
    directory = dataSetsEtl[title]['info']['directory']
    datasetCharId = dataSetsEtl[title]['info']['datasetCharId']
    filetr = dataSetsEtl[title]['info']['filetr']
...
getETL( "Recently Expired and Surrendered Liquor Licenses in Colorado")
...
for title,infos in dataSetsEtl.items():
    print(title)
    print("   ",infos['info'])
    print("   ",infos['extract'])    
    print("   ",infos['transform'])    
...
for file in runEtls:
    ...
    for a in data:
      #  print(type(a))
        if "extract" in a:
            if isinstance(a["extract"],dict):
               extractJob =  extractDecode(a["extract"])
            #   print("      ",extractJob)
            elif isinstance(a["extract"],list):
                extractJob=""
                for e in a["extract"]:
                    extractJob+=  extractDecode(e) + ";"
             #   print(extractJob)
        if "transform" in a:
...
## Build actions
    ...
    if 'extract' in dataSetsEtl[k]:
        print("DD ",dataSetsEtl[k])
        if isinstance(dataSetsEtl[k]["extract"],dict):
            extract =  extractDecode(dataSetsEtl[k]["extract"])
           
        elif isinstance(dataSetsEtl[k]["extract"],list):
            extract=""
            for e in dataSetsEtl[k]["extract"]:
                extrac+=  extractDecode(e) + ";"

    transform=""
    filetr = ""
    print("Get Transform")
    
    if 'transform' in dataSetsEtl[k]:
        
        if dataSetsEtl[k]["transform"]["language"] == "node":
             ff=dataSetsEtl[k]["transform"]["file"]
             transform=f"node {bicHome}{directory}/{ff}"
        elif dataSetsEtl[k]["transform"]["language"] == "py":
             ff=dataSetsEtl[k]["transform"]["file"]
             transform=f"PIPENV_PIPFILE=/home/joe/bic_etl/cdor/regulations_liquor/scripts/Pipfile pipenv run python {bicHome}{directory}/{ff}"
     #   filetr=ff    
        options=""
        if 'options' in dataSetsEtl[k]['transform']:
            for opts in dataSetsEtl[k]['transform']['options']:
                options+= f" {opts}" 
                if isinstance(opts,str):
                    spl = opts.split(" ")
                    if "-i" in spl[0]:
...
a,e,t,f,d,d2 = runETLNew("Recently Expired and Surrendered Liquor Licenses in Colorado")
print("Ex ",e)
