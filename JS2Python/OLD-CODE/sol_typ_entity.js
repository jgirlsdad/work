...
function initiate() {
    ...
    let file_path = (program.folderpath) ? program.folderpath : path.join(process.env.bic_etl_home, 'cdos/business/nonprofit', 'data_source', 'sol_typ_entity.tsv');
...
//  Note there is an fein column in source data, but it appears to always be missing for this dataset
    let tRow                    = {};
    tRow["entityId"]           = row[entity_id_input];
    tRow["documentId"]             = row["Document Id"];
    tRow["orgName"]            = row["Org Name"];
    tRow["solTypeDscrp"]      = row["Sol Type Dscrp"];
    return tRow;
}
