...
def getFilesClicked(event,values,window):
        print("Get Files")
        print(event)
...
def fileBrowser():
    ...
    layout = [[sg.Button("Close")],
          ##   [sg.Button("Compare")],
             [sg.FilesBrowse(button_text="Read File",initial_folder="/home/joe/bic_etl",font="CENTAUR 15",file_types=[("CSV Files","*.csv"),("TSV Files","*.tsv"),("Excel Files","*.xlsx"),("Excel Files","*.xls"),("SIJ Files","*.sij")],enable_events=True,key='-FILE1-')],
             [sg.Text("Web Address:",font="CENTAUR 15"),sg.Input(default_text="",font="CENTAUR 15",key="-WEB1-"),sg.Button("Fetch")], 
             [sg.Checkbox("Show Recrods",font="CENTAUR 15",key="-SHOWRECS-")],
    ...
    layout2 = [[sg.Column(layout,vertical_scroll_only=False,scrollable=True)]]
    window = sg.Window('Files',layout2,finalize=True,resizable=True)
...
#                    values["-FILE1-"] = "/home/joe/bic_etl/cdos/business/nonprofit/data_transformed/sol_typ_entity.csv"
            ...
            #        file="/home/joe/work/COURT/CountyCourtFilings.csv"
                    ...
                    if (event == "-FILE1-" and values["-FILE1-"][-4:].lower() != ".sij") or event == "Fetch":
                        ...
                        df1,stats1,file1 = getFilesClicked(event,values,window)
...
import pandas as pd
df = pd.read_csv("/home/joe/bic_etl/cdos/lobbyist/data_source/prof_bills.txt",encoding="latin",delimiter="\t")


# In[20]:


fin = open("/home/joe/bic_etl/cdos/lobbyist/data_source/prof_bills.txt",encoding="latin")
nline=1
hist = {}
...
for nn,line in enumerate(lines):
    ...
    if len(spl) < 16:
        ...
        mm=nn
        newline=""
        while len(lines[mm].split("\t")) < 16 and mm not in done and len(newline.split("\t")) < 16:
            done[mm]=1
            newline+=lines[mm].rstrip()
            print("  ",mm,len(lines[mm].split("\t")))
            mm+=1
