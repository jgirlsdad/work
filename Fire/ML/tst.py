...
# for nn in range(10):
#     model,y_test,yhat,yrmo_test,mos_test,hist=predicting(features,wxData,yrmos,mos,weights,now)
...
def main(total_tasks=50, max_procs=10):
    # Prepare the inputs for 50 tasks
   # files=glob.glob("Data/*.grib")
    files=["3da25100a42c035414eef7dde0b27070.grib","91894e9252f4bfe9946363c271068844.grib","9035aad105e492216740525c32c4eeee.grib"]




# #   files = open("/home/joe/Fire/list2").readlines()
#    files = [f.strip() for f in files]
#    files=files[:11]
    # print(files)
    # # total_tasks = len(files)
    # # print("Tasks: ", total_tasks)
    # task_ids = list(range(0, total_tasks))
    # invt={}
    # invtVar={}
    # fout=open("files_summary3.txt","w")
   
    # Create a process pool with up to 10 workers
    with ProcessPoolExecutor(max_workers=max_procs) as executor:
        # Submit all tasks at once; the executor will run at most max_procs concurrently
  #      futures = {executor.submit(work,f"/home/joe/Fire/{files[nn]}",invt): files[nn] for nn in range(len(files))}
        futures = {executor.submit(predicting, features, wxData, yrmos, mos, weights, now): nn for nn in range(10)}

        # As each future completes, we handle results/errors and the pool picks up queued tasks
...
if __name__ == "__main__":
    # On Windows/macOS, the __main__ guard is required for multiprocessing
    weights=None
    df=readWxData('/home/joe/work/Fire/Data/DB/era5DataMeans.db')
    wxData,yrmos,mos=selectWxData(df,500,"VPD")
    now=datetime.datetime.today()
    main(total_tasks=3, max_procs=3)
    now1=datetime.datetime.today()
