...
# -------------------------------
# Step 1: Compute monthly biases on raw validation forecasts
# -------------------------------
biases = {}
...
# -------------------------------
# Step 2: Compute monthly ratios (using bias-corrected forecasts)
# -------------------------------
val_bias_corrected = []
val_obs = []
for i in range(len(val_preds_inv)):
...
# -------------------------------
# Step 3: Compute new biases after applying ratio correction on validation set
# -------------------------------
post_ratio_biases = {}
for month in range(1, 13):
    diffs = []
    for i in range(len(val_preds_inv)):
        ...
        for h in range(HORIZON):
            ...
            if verif_date.month == month:
                ...
                diffs.append(y_val_inv[i, h] - ratio_corrected)
    if len(diffs) > 0:
        post_ratio_biases[month] = float(np.mean(diffs))

# -------------------------------
# Step 4: Apply corrections to test set
# -------------------------------
rows = []
for i in range(len(test_preds_inv)):
    ...
    for h in range(HORIZON):
        ...
        bias = biases.get(verif_date.month, 0.0)
        ratio = ratios.get(verif_date.month, 1.0)
        post_ratio_bias = post_ratio_biases.get(verif_date.month, 0.0)

        bias_corrected = raw + bias
        ratio_corrected = (raw + bias) * ratio     # bias → ratio
        ratio_then_bias = (raw * ratio) + post_ratio_bias  # ratio → bias
        ...
        rows.append({
            ...
            "ratio_then_bias_pred": ratio_then_bias
        })

out = pd.DataFrame(rows)
out.to_csv("lstm_monthly_ratio_then_bias_predictions.csv", index=False)
...
for month, subset in out.groupby("month"):
    ...
    mae_rtb, rmse_rtb = compute_metrics(subset["obs"], subset["ratio_then_bias_pred"])
    monthly_rows.append({
        "month": month,
        "raw_mae": mae_raw, "raw_rmse": rmse_raw,
        "bias_mae": mae_bias, "bias_rmse": rmse_bias,
        "ratio_mae": mae_ratio, "ratio_rmse": rmse_ratio,
        "ratio_then_bias_mae": mae_rtb, "ratio_then_bias_rmse": rmse_rtb
    })
...
print("Saved predictions → lstm_monthly_ratio_then_bias_predictions.csv")
